...
let kCircleHeightBackground: CGFloat = SwifterSwift.isPad ? 80 : 48.0
...
open class SCLAlertView: UIViewController {
    ...
    public struct SCLAppearance {
        ...
        // Fonts
        let kTitleFont: UIFont = UIFont.boldSystemFont(ofSize: SwifterSwift.isPad ? 23 : 16)
        let kTextFont: UIFont = UIFont.systemFont(ofSize: SwifterSwift.isPad ? 20 : 14)
        let kButtonFont: UIFont = UIFont.systemFont(ofSize: SwifterSwift.isPad ? 22 : 15)
        ...
        var buttonCornerRadius : CGFloat = 22.5
        var dynamicAnimatorActive : Bool
        var buttonsLayout: SCLAlertButtonLayout = .horizontal
        ...
        public init(kDefaultShadowOpacity: CGFloat = 0.6, kCircleTopPosition: CGFloat = 0.0, kCircleBackgroundTopPosition: CGFloat = 6.0, kCircleHeight: CGFloat = 0.0, kCircleIconHeight: CGFloat = SwifterSwift.isPad ? 50: 28.0, kTitleTop:CGFloat = 30.0, kTitleHeight:CGFloat = 25.0,  kWindowWidth: CGFloat = SwifterSwift.isPad ? 530 : 270, kWindowHeight: CGFloat = SwifterSwift.isPad ? 360 :190, kTextHeight: CGFloat = 90.0, kTextFieldHeight: CGFloat = 45.0, kTextViewdHeight: CGFloat = 80.0, kButtonHeight: CGFloat = 40.0, kTitleFont: UIFont = UIFont.systemFont(ofSize: 20), kTitleMinimumScaleFactor: CGFloat = 1.0, kTextFont: UIFont = UIFont.systemFont(ofSize: 14), kButtonFont: UIFont = UIFont.boldSystemFont(ofSize: 14), showCloseButton: Bool = true, showCircularIcon: Bool = true, shouldAutoDismiss: Bool = true, contentViewCornerRadius: CGFloat = 0, fieldCornerRadius: CGFloat = 3.0, buttonCornerRadius: CGFloat = 17.5, hideWhenBackgroundViewIsTapped: Bool = false, circleBackgroundColor: UIColor = UIColor.white, contentViewColor: UIColor = UIColorFromRGB(0xFFFFFF), contentViewBorderColor: UIColor = UIColorFromRGB(0xCCCCCC), titleColor: UIColor = UIColorFromRGB(0x4D4D4D), dynamicAnimatorActive: Bool = false, disableTapGesture: Bool = false, buttonsLayout: SCLAlertButtonLayout = .horizontal, activityIndicatorStyle: UIActivityIndicatorViewStyle = .white) {
    ...
    fileprivate func setup() {
        ...
        circleBG.layer.shadowOpacity = 1
        circleBG.layer.shadowOffset = CGSize(width: 0.4, height: 0.4)
        circleBG.layer.masksToBounds = false
    ...
    override open func viewWillLayoutSubviews() {
        ...
        contentView.frame = CGRect(x:x, y:y, width:appearance.kWindowWidth, height: SwifterSwift.isPad ? 60 + windowHeight : windowHeight)
        ...
        labelTitle.frame = labelTitle.frame.offsetBy(dx: 0, dy: SwifterSwift.isPad ? titleOffset + 20 : titleOffset)
        ...
        viewText.frame = CGRect(x:hMargin, y: SwifterSwift.isPad ? y + 20 : y, width: viewTextWidth, height:viewTextHeight)
        // Text fields
        ...
        let widthEachButton = (appearance.kWindowWidth - 34) / numberOfButton
        var buttonX = numberOfButton > 1 ? CGFloat(12) : CGFloat(80)
        let gap = 24 + (10 * numberOfButton - 1)
        let buttonWidth = numberOfButton > 1 ? CGFloat((SwifterSwift.isPad ? 530 - gap : 270 - gap) / numberOfButton) : appearance.kWindowWidth - 160
        switch appearance.buttonsLayout {
        case .vertical:
            for btn in buttons {
                btn.frame = CGRect(x:80, y:y, width:appearance.kWindowWidth - 160, height: 35)
        ...
        case .horizontal:
            for btn in (0..<buttons.count) {
                if btn == 0 && numberOfButton > 1{
                    buttons[btn].frame = CGRect(x:buttonX, y: SwifterSwift.isPad ? y + 35 :  y, width: buttonWidth, height: SwifterSwift.isPad ? 55 : 35)
                    buttons[btn].backgroundColor = UIColor.white
                    buttons[btn].layer.borderColor = AppDelegate.shared.appThemeColor.cgColor
                    buttons[btn].layer.borderWidth = 0.8
                    buttons[btn].setTitleColor(AppDelegate.shared.appThemeColor, for: .normal)
                } else {
                    let btnTitle = buttons[btn].titleLabel?.text ?? ""
                    if btnTitle.count <= 8{
                        buttons[btn].frame = CGRect(x:buttonX, y: SwifterSwift.isPad ? y + 35 :  y, width: buttonWidth, height: SwifterSwift.isPad ? 55 : 35)
                    } else {
                        let size = btnTitle.size(withAttributes:[.font: UIFont.systemFont(ofSize: 15)])
                        let btnX = appearance.kWindowWidth - size.width - 30
                        buttons[btn].frame = CGRect(x:btnX / 2, y: SwifterSwift.isPad ? y + 35 :  y, width: size.width + 30, height: SwifterSwift.isPad ? 55 : 35)
                    }
                }
    ...
    @discardableResult
    fileprivate func addButton(_ title:String, backgroundColor:UIColor? = nil, textColor:UIColor? = nil, isSecondButton:Bool? = nil, showTimeout:SCLButton.ShowTimeoutConfiguration? = nil)->SCLButton {
        // Update view height
    ...
    @discardableResult
    open func showCustom(_ title: String, subTitle: String, color: UIColor? = nil, icon: UIImage, closeButtonTitle:String?=nil, timeout:SCLTimeoutConfiguration?=nil, colorStyle: UInt=SCLAlertViewStyle.success.defaultColorInt, colorTextButton: UInt=0xFFFFFF, circleIconImage: UIImage? = nil, animationStyle: SCLAnimationStyle = .topToBottom ) -> SCLAlertViewResponder {
        return showTitle(title, subTitle: subTitle, timeout: timeout, completeText:closeButtonTitle, style: .success, colorStyle: SCLAlertViewStyle.success.defaultColorInt, colorTextButton: colorTextButton, circleIconImage: icon, animationStyle: animationStyle)
    }
    ...
    open func showTitle(_ title: String, subTitle: String, timeout: SCLTimeoutConfiguration?, completeText: String?, style: SCLAlertViewStyle, colorStyle: UInt?=0x000000, colorTextButton: UInt?=0xFFFFFF, circleIconImage: UIImage? = nil, animationStyle: SCLAnimationStyle = .topToBottom) -> SCLAlertViewResponder {
        ...
        case .error:
            ...
            iconImage = #imageLiteral(resourceName: "ic_logout_alert")
